...
\ifnum#2=2%
    \tikz[baseline=3pt]{%(tmp2.center)]{
        ...
        \fi
        \ifnum#4>0
          \node[#1, circle,inner sep=0.5pt, fill=black, draw=white] (tmp1) at (0,16pt){\footnotesize 1};
        \else
          \node[#1, circle,inner sep=2pt, fill] (tmp1) at (0,16pt){};
        \fi
        \ifnum#4>1
          \node[#1,circle,inner sep=0.5pt, fill=black, draw=white] (tmp2) at (0,-4pt){\footnotesize 2};
        \else 
...
\newcommand{\FlagGraphLabelThree}[5][very thick]{
  \tikz[baseline=-2pt]{
        ...
        \else 
          \node [thick, circle,inner sep=1pt, fill=black, draw, minimum size = 3pt] at (tmp.corner 1){\footnotesize $#3$};
        \fi
        \ifx#40
          \node [#1,circle,inner sep=2pt, fill] at (tmp.corner 2){};
        \else 
          \node [thick, circle,inner sep=1pt, fill=black, draw, minimum size = 3pt] at (tmp.corner 2){\footnotesize $#4$};
        \fi
        \ifx#50
          \node [#1,circle,inner sep=2pt, fill] at (tmp.corner 3){};
        \else 
          \node [thick, circle,inner sep=1pt, fill=black, draw, minimum size = 3pt] at (tmp.corner 3){\footnotesize $#5$};
        \fi
    }
}
